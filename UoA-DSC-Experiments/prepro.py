...
try:
    ...
    print('warning: pydicom is not installed, so you can load the png files,'
          'but not the .dcms in the large dataset ')
...
def preprocess_image(img, res=(256, 256)):
    """Set image to desired resolution and normalise for mean 0 and std 1"""
...
def basic_load(setpath, res=(256, 256), img_dir="CXR_png",
    ...
    for (imname, labname) in zip(imgfiles, labelfiles):
        print("loading image: " + str(imname) +
              ", and labels: " + str(labname))
...
def match_dicoms(base_dir, labels_path):
...
def load_dicoms(base_dir, labels_path, allowed_views=["LL"], res=(256, 256)):
    ...
    for pair in labeled_paths[4:]:
        ...
        try:
            ...
            print("key error on file: %s" % path)
    X = np.stack(X)
